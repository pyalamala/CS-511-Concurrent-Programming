...
do_propegate_message(State, Ref, ClientPID, Message) ->
    case maps:get(ClientPID, State#chat_st.registrations, undefined) of
		undefined -> error;
		ClientNick ->
			
			lists:map(fun (Client) -> 
							Client ! {request, self(), Ref, {incoming_msg, ClientNick, State#chat_st.name, Message}}
							end, lists:filter (fun (PID) ->
													PID =/= ClientPID
													end, maps:keys(State#chat_st.registrations))),
			ClientPID ! {self(), Ref, ack_msg},
			NewState = State#chat_st{name = State#chat_st.name, registrations = State#chat_st.registrations, history = State#chat_st.history ++ [{ClientNick, Message}]},
			NewState
	end.
