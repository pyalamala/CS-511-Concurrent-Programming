...
do_join(State, Ref, ChatName) ->
    case maps:get(ChatName, State#cl_st.con_ch, undefined) of
		undefined ->
			whereis(server) ! {self(), Ref, join, ChatName},
			receive
				{From, Ref, connect, History} -> {History, State#cl_st{con_ch = maps:put(ChatName, From, State#cl_st.con_ch)}}
			end;
		_PID -> {err, State}
		end.
...
do_msg_send(State, Ref, ChatName, Message) ->
    case maps:get(ChatName, State#cl_st.con_ch, undefined) of
		...
		ChatPID -> 
			...
			receive
				{_From, Ref, ack_msg} -> {{msg_sent, State#cl_st.nick}, State};
				_ -> {err, State}
			end
	end.
